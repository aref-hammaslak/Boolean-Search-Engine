...
export const SearchField = ({
  ...
  return (
    <div className='bg-primary-content px-4 py-2 rounded'>
      <div className="mb-2">
        <span className="text font-bold mr-2 text-lg">
          Insert:
        </span>
        {
        [["New Word", ' " "'], ["Patentheses", ' ( )'], ["AND", ' and'], ["OR", ' or'], ["NOT", ' not'],].map(([lable, operator]) => {

          return (
            <button className="btn- px-2 py-1  font-semibold rounded btn-secondary btn-outline" onClick={() => setText(text + operator)}>
              {lable}
            </button>
          )
        })
